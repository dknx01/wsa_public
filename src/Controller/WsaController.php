...
class WsaController extends AbstractController
{
    ...
    public function home(DocumentsRepository $repository): Response
    {
        ...
        return $this->render('wsa/index.html.twig', ['states' => $allBundeslaender]);
    }
    ...
    public function getByState(string $state, DocumentsRepository $repository, Packages $packages): JsonResponse
    {
        ...
        /** @var Document $landesliste */
        $landesliste = $repository->findAllByState($state)[0] ?? null;
        if (null !== $landesliste) {
            $data[] = new DocumentResponse(
                $landesliste->getName(),
                $packages->getUrl('images/sticker.png'),
                nl2br((string) $landesliste->getDescription()),
                $this->generateUrl('documentDownload', ['id' => $landesliste->getId()]),
                $state
            );
        }
        /** @var Document $document */
        foreach ($repository->findAllByState($state, 'Direktkandidaten') as $document) {
            $data[] = new DocumentResponse(
                $document->getName(),
                $packages->getUrl('images/sticker.png'),
